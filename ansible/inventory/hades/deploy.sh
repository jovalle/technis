...
export K3S_OPTIONS="--flannel-backend none --disable-kube-proxy --disable-network-policy --disable servicelb --etcd-expose-metrics true"
export K3S_HOSTS=(
  cerberus
  thanatos
  zagreus
...
export K3S_VERSION="v1.29.2+k3s1"
...
which "k3sup" &>/dev/null || (curl -sLS https://get.k3sup.dev | sh) || sudo install k3sup /usr/local/bin/
...
# for server in ${K3S_SERVERS[@]:2}; do
